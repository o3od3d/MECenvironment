...
class doubleAuction():
    ...
    def doubleAuctionConstraints(self):
        ...
        ISD_temp_ask = dict()
        for i in range(len(self.win_ISD)):
            falseAsk = round(random.uniform((self.win_ISD[self.isd_index[i]]['ask'] + 0.001), 10), 3)
            ISD_temp_ask[self.isd_index[i]] = {'True_ask':self.win_ISD[self.isd_index[i]]['ask'], 'False_ask for case1': falseAsk}

        # ISD case 2 = c(true) > a(false)
        for i in range(len(self.win_ISD)):
            falseAsk = round(random.uniform(3, (self.win_ISD[self.isd_index[i]]['ask'] - 0.001)), 3)
